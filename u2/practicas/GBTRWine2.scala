...
val df = original.select($"points", $"variety", $"province", $"region_1", $"price")
...
val regionIndexer = new StringIndexer().
    setInputCol("region_1").
    setOutputCol("regionIndex")


//We define the assembler to collect the columns into a new column with a single vector - "features"
val assembler = new VectorAssembler().
    setInputCols(Array("points", "varietyIndex", "provinceIndex", "regionIndex")).
